...
export default function SkillRadarChart() {
  return (
    <div className="bg-[#151c2c] border border-gray-800 rounded-xl p-6 flex flex-col justify-between">
      <div>
        <h3 className="text-white font-semibold mb-2">Độ phủ kiến thức cú pháp</h3>
        <div className="relative w-full h-52 flex items-center justify-center">
          <span className="absolute top-1 text-[10px] text-gray-400">Khai báo biến (let, const)</span>
          <span className="absolute right-0 top-1/3 text-[10px] text-gray-400">Hàm (Function)</span>
          <span className="absolute right-2 bottom-1/4 text-[10px] text-gray-400">Xử lý Chuỗi (String)</span>
          <span className="absolute bottom-1 text-[10px] text-gray-400">Vòng lặp (for, while)</span>
          <span className="absolute left-2 bottom-1/4 text-[10px] text-gray-400">Điều kiện (if-else)</span>
          <span className="absolute left-2 top-1/3 text-[10px] text-gray-400">Hàm mảng (.map, .filter)</span>
